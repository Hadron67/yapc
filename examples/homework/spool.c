...
int YTree_init(YTree *tree,int size,size_t elemsize,ycomparator comp,void *arg,yheap_t *heap){
    ...
    tree->buf = (YTNode *)ya_malloc(heap,tree->size * tree->nodeSize);
...
static int YTree_prepareNode(YTree *tree){
    if(tree->len >= tree->size){
        tree->size *= 2;
        tree->buf = (YTNode *)ya_realloc(tree->heap,tree->buf,tree->nodeSize * tree->size);
    }
    return 0;
}
...
static int YSPool_comp(const void *d1,const void *d2,void *arg){
    YSPool *p = (YSPool *)arg;
    const YS *s1 = (const YS *)d1;
    const YSPoolEntry *s2 = (const YSPoolEntry *)d2;
    return strcmp(s1->s,p->buf + s2->s);
}

int YSPool_init(YSPool *pool,size_t bufSize,size_t nBufSize,yheap_t *heap){
    memset(pool,0,sizeof(YSPool));
    pool->heap = heap;
    pool->bufSize = bufSize;
    pool->buf = (char *)ya_malloc(pool->heap,bufSize * sizeof(char));
    YTree_init(&pool->tree,nBufSize,sizeof(YSPoolEntry),YSPool_comp,pool,heap);
    return 0;
}
int YSPool_free(YSPool *pool,char **s){
    ...
    if(s != NULL){
        ...
        *s = pool->buf;
        ...
        ya_free(pool->heap,pool->buf);
        return 0;
    }
}

static ysptr YSPool_addS(YSPool *pool,const char *s,int len){
    //inserting string
    ...
    while(len --> 0){
        if(pool->bufLen >= pool->bufSize){
            pool->bufSize *= 2;
            pool->buf = (char *)ya_realloc(pool->heap,pool->buf,pool->bufSize);
        }
...
ysptr YSPool_addStringN(YSPool *pool,const char *s,int len){
    ynptr node;
    YS ys;
    ys.s = s;
    ys.len = len;
    ynptr *pos = YTree_findEX(&pool->tree,&ys,&node);
    if(*pos == YTNODE_NULL){
        YSPoolEntry *ret = (YSPoolEntry *)YTree_insertAt(&pool->tree,node,pos);
        ret->len = len;
        ret->s = YSPool_addS(pool,s,ret->len);
        return ret->s;
    }
    else{
        return ((YSPoolEntry *)YTree_getNode(&pool->tree,*pos)->data)->s;
    }
}
ysptr YSPool_addString(YSPool *pool,const char *s){
    return YSPool_addStringN(pool,s,strlen(s));
}
